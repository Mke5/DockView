...
import { RegistryAccount, RegistryRepo, RegistryType } from '../../store';
...
export function AddRegistryModal({
  ...
  const [type, setType] = useState<RegistryType>('dockerhub');
    ...
    <Modal title="Add Registry Node" onClose={onClose}>
      <div className="space-y-6">
        <ModalField
          ...
          description="Select the registry host service"
        >
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              <button
                key={t.id}
                onClick={() => setType(t.id as RegistryType)}
        ...
        <ModalField
          label="Display Name"
          description="Internal identifier for this node"
        >
          <ModalInput
            value={name}
            onChange={setName}
        ...
        <ModalField
          label="Namespace / Organization"
          description="Your primary scope on this registry"
        >
          <ModalInput
            value={namespace}
...
export function PushImageModal({
  ...
  onPushed: (accountId: string, repo: { name: string }) => void;
}) {
  ...
  return (
    <Modal title="Push Image To Registry" onClose={onClose}>
      <div className="space-y-6">
        <ModalField
          label="Target Node"
          description="Registry where the image will be stored"
        >
            ...
            {accounts.map((acc) => (
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  />
                  <span
                    className={`text-[12px] font-bold ${selectedAcc === acc.id ? 'text-brand' : 'text-text-primary'}`}
                  >
                    {acc.name}
                  </span>
                </div>
                <Badge variant="gray" className="text-[8px] font-black">
                  {acc.namespace}
                </Badge>
              </button>
            ))}
        ...
        <ModalField
          ...
          description="Final path in the registry"
        >
          <ModalInput
            value={imageName}
        ...
        <div className="p-4 rounded-xl bg-surface-sunken border border-border-subtle flex gap-3">
          ...
          <p className="text-[10px] text-text-disabled leading-relaxed">
            Ensure you have the required permissions to push to this namespace.
            Deployment tags will be requested in the next step.
          </p>
        </div>
    ...
    <Modal title="Pull From Registry" onClose={onClose}>
      <div className="space-y-6">
        <div className="flex items-center gap-4 p-4 rounded-2xl bg-surface-sunken border border-border-subtle">
          ...
          <div>
            <h3 className="text-[14px] font-black text-text-primary uppercase tracking-tight leading-none">
              {repo.name}
            </h3>
            <span className="text-[10px] font-mono text-text-disabled mt-2 inline-block leading-none">
              {account.namespace}
            </span>
          </div>
        </div>
        ...
        <ModalField
          ...
          description="Select the specific image tag to pull"
        >
          <div className="grid grid-cols-1 gap-2">
            {repo.tags.map((tag) => (
              ...
              >
                <span
                  className={`text-[12px] font-mono font-bold ${selectedTag === tag.name ? 'text-brand' : 'text-text-primary'}`}
                >
                  {tag.name}
                </span>
...
export function CreateRepoModal({
  ...
  onClose: () => void;
  account: RegistryAccount;
  onCreated: (repo: { name: string; isPrivate: boolean }) => void;
}) {
  const [name, setName] = useState('');
      ...
      <div className="space-y-6">
        <div className="p-4 rounded-xl bg-brand/5 border border-brand/20 flex gap-3">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[12px] font-bold text-text-primary leading-none">
              {account.name} / {account.namespace}
            </p>
          </div>
        </div>
        ...
        <ModalField
          ...
          description="Final URI segment for the container artifact"
        >
          <ModalInput
            value={name}
            onChange={setName}
        ...
        <ModalField
          ...
          description="Configure manifest accessibility scope"
        >
